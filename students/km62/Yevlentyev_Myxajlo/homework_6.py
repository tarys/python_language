...
# 4)Дан список чисел. Если в нем есть два соседних элемента одного знака, выведите эти числа.
...
# 5)Дан список чисел. Выведите значение наибольшего элемента в списке, а затем индекс этого элемента в списке.
...
classmates = [int(i) for i in input().split()]
piter = int(input())
 
num = 1
classmates.sort(reverse=True)
for i in range(len(classmates)):
    if classmates[i] >= piter:
        num = i + 2
